...
const Message = ({ message }) => {
  ...
  const { user } = useAuth();
  const isUserMessage = username === user.username;

  return (
    <div key={id} className="text-break mb-2">
      <b className={isUserMessage ? 'text-white bg-dark' : ''}>{username}</b>
      : {body}
    </div>
  );
};

const ChannelInfo = ({ currentChannel, count }) => {
...
const ChatInput = ({ currentChannelId }) => {
...
const Messages = () => {
  ...
  return (
    <Col className="col-9 p-0 d-flex flex-column h-100">
      <ChannelInfo currentChannel={currentChannel} count={count} />
